...
int test_charset_table(string t) {
  ...
  foreach(names, string name)
    if(name!=normalize(name)) {
      write("%O is not correctly normalized into %O in %s.\n",
...
int test_generic_debug(string dir)
{
  int failures = 0;
...
int main(int args)
{
  ...
  int failures;
  failures += test_manifest();

  failures += test_constants();
  failures += test_copyright();
  failures += test_copying();
  failures += test_master_year();
  failures += test_install_year();
  ...
  failures += test_fncases("src");
  failures += test_fncases("lib");
  failures += test_fncases("bin");
  failures += test_fncases("man");
  failures += test_fncases("tools");
  failures += test_fncases("packaging");
  /* failures += */ test_fncases("refdoc");
